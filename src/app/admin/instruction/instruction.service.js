	...
	function InstructionService ($rootScope, $http, $q, $cacheFactory, ngProgressFactory, Upload, API_HOST, FILE_HOST) {
		...
		function InstructionService(){
			...
			self.get = function (request) {
				...
				progress.start();
				$http.get(API_HOST + '/instruction/paginate/' + request.perPage + '?page=' + request.currentPage + '&keyword=' + $rootScope.encodeURI(request.keyword))
			...
			self.validatingNo = function(request) {
				...
				var progress = ngProgressFactory.createInstance();
				progress.start();
				$http.post(API_HOST + '/instruction/validating/no', request)
					.then(function(response){
						progress.complete();
						deferred.resolve(response.data)
					}, function(data) {
			...
			self.validatingDescription = function(request) {
				...
				var progress = ngProgressFactory.createInstance();
				progress.start();
				$http.post(API_HOST + '/instruction/validating/description', request)
					.then(function(response){
						progress.complete();
						deferred.resolve(response.data)
					}, function(data) {
